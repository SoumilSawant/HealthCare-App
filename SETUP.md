...
- ✅ LoginScreen (Screen 02) with password + OTP option
- ✅ SignupScreen with OTP verification
...
def send_otp(mobile_number):
    ...
    # 3. Store in OTP DocType with expiry (5 minutes)
    # 4. Send via SMS provider (Twilio, AWS SNS, etc.)
    
    otp = frappe.get_doc({
        'doctype': 'OTP',
        'mobile_number': mobile_number,
        'otp': str(frappe.utils.get_random(0000, 9999)),
        'created_at': now(),
        'expires_at': frappe.utils.add_to_date(now(), minutes=5),
        'status': 'pending'
    })
    otp.insert()
    
    # Send SMS here
    send_sms_otp(mobile_number, otp.otp)
...
@frappe.whitelist(allow_guest=True)
def verify_otp_and_signup(mobile_number, otp, user_data):
    """Verify OTP and create new patient user"""
    # 1. Verify OTP
    otp_doc = frappe.get_value(
        'OTP',
        filters={'mobile_number': mobile_number, 'status': 'pending'},
        fieldname=['name', 'otp', 'expires_at']
    )
    
    if not otp_doc or otp_doc[1] != otp:
        frappe.throw(_('Invalid OTP'))
    
    if frappe.utils.now_datetime() > otp_doc[2]:
        frappe.throw(_('OTP expired'))
    
    # 2. Mark OTP as used
    frappe.db.set_value('OTP', otp_doc[0], 'status', 'verified')
    
    # 3. Create Patient User
    patient = frappe.get_doc({
        'doctype': 'Patient User',
        'mobile_number': mobile_number,
        'full_name': user_data['name'],
        'age': user_data['age'],
        'gender': user_data['gender'],
        'living_status': user_data['living_status'],
...
@frappe.whitelist(allow_guest=True)
def verify_otp_and_login(mobile_number, otp):
    """Verify OTP for login"""
    # Similar to signup OTP verification
    # Return user session
